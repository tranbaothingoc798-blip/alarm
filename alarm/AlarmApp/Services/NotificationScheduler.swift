...
final class NotificationScheduler {
    ...
    /// Schedules notifications for a single alarm. Cancels any existing notifications for this alarm first.
    func scheduleAlarm(_ alarm: Alarm, context: ModelContext) async {
        guard alarm.isEnabled else { return }
        await cancelPendingAlarms(for: alarm.id)
        let budgeted = budgeter.budgetedOccurrences(from: [alarm])
        for (a, date) in budgeted {
            for sec in budgeter.escalations {
                let triggerDate = date.addingTimeInterval(TimeInterval(sec))
                let content = UNMutableNotificationContent()
                content.title = a.label.isEmpty ? "Alarm" : a.label
                content.body = sec == 0 ? "Mission required to dismiss" : "Escalation \(sec)s"
                content.sound = .default
                let identifier = "\(alarmPrefix)\(a.id.uuidString)_\(Int(triggerDate.timeIntervalSince1970))_e\(sec)"
                content.userInfo = ["alarmID": a.id.uuidString, "escalation": sec, "requestId": identifier]
                let comps = Calendar.current.dateComponents([.year, .month, .day, .hour, .minute, .second], from: triggerDate)
                let request = UNNotificationRequest(identifier: identifier, content: content, trigger: UNCalendarNotificationTrigger(dateMatching: comps, repeats: false))
                try? await center.add(request)
                EventLogger.shared.logJSON(.alarmScheduled, alarmID: a.id, payload: ["scheduledFor": isoString(date), "triggerDate": isoString(triggerDate), "escalationSec": sec, "requestId": identifier, "alarmId": a.id.uuidString, "type": "alarm"], context: context)
            }
        }
        cachedPendingCount = await pendingCount()
    }

    /// Cancels all pending notifications for an alarm. Idempotent.
